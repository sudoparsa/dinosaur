...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Visualize Slots masks')
    ...
    parser.add_argument("--image_size", default=(100, 100),
                        type=int, nargs="+", help="Resize image.")
    ...
    attentions = output['decoder_output']['masks']
    print(f'Mask Shape: {attentions.shape}')

    nh = attentions.shape[1]  # number of head

    # we keep only the output patch attention
    attentions = attentions[0, :, 0].reshape(nh, -1)
    print(attentions.shape, nh, w_featmap, h_featmap)
    ...
    torchvision.utils.save_image(torchvision.utils.make_grid(
        img, normalize=True, scale_each=True), os.path.join(args.output_dir, "img.png"))
    for j in range(nh):
        fname = os.path.join(args.output_dir, "slot" + str(j) + ".png")
    ...
    if args.threshold is not None:
        ...
        for j in range(nh):
            display_instances(image, th_attn[j], fname=os.path.join(
                args.output_dir, "mask_th" + str(args.threshold) + "_head" + str(j) + ".png"), blur=False)
